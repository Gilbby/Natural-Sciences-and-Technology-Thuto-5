...
const topic = (id: string, title: string, emoji: string, weeks: number, text: string, factRef: string, choices: [string, string, string, boolean][], order: [string, string][], kit: string[] = ['paper']) => buildTopic({
  id, term: 1, weeks: ATP_RANGES[id] ?? [1, Math.max(1, Math.ceil(weeks))], atpWeeks: weeks, area: 'FAL', atpHours: 0, requires: [], teaches: [], skills: ['observe', 'compare', 'communicate'], title, emoji,
  ...
  lessons: [
    { id: 'meet', title: `Meet ${title}`, emoji, activities: [
      readText('a1', 'Read and listen.', { mode: 'app-first', textType: 'information', title, emoji, factRef, lines: [['l1', text], ['l2', 'Look closely, compare what you see and say the idea in your own words.'], ['l3', 'Good reading starts with looking carefully at the text.']] }),
      select('a2', 'Tap the true idea.', 'one', choices),
      sequence('a3', 'Put the steps in order.', order.map(([e, l], i) => [`${i}`, e, l] as [string, string, string])),
    ...
    { id: 'do', title: 'Look and do', emoji: '🔎', activities: [
      select('b1', 'Tap the best reading action.', 'one', [['a', '👀', 'read carefully', true], ['b', '🙈', 'guess without looking', false]]),
      ...
      readText('b3', 'Read the reminder.', { mode: 'app-first', textType: 'information', title: 'Reading reminder', emoji: '💡', lines: [['l1', 'Read the text and record what you find.'], ['l2', 'Look for what the text tells you about this topic.']] }),
